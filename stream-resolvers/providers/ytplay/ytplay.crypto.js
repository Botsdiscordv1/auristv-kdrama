...
const { PROTOCOL, IV_CANDIDATES } = require('./ytplay.constants');

const ALGO = 'aes-128-cbc';

/**
 * Decrypts an AES-CBC hex payload produced by the YTPlay /api/v1/video
...
function decryptPayload(payload, options = {}) {
  ...
  if (typeof payload !== 'string') {
    throw new TypeError('YTPlay payload must be a string');
  }
  const hex = String(payload).replace(/\s+/g, '');
  if (!hex) {
    throw new Error('YTPlay payload is empty');
  }
  if (hex.length % 2 !== 0) {
    throw new Error('YTPlay payload is not valid hex (odd length)');
  }
  const ciphertext = Buffer.from(hex, 'hex');
  if (ciphertext.length === 0) {
    throw new Error('YTPlay payload produced no ciphertext bytes');
  }
  ...
  throw new Error(`YTPlay payload could not be decrypted: ${lastError ? lastError.message : 'no candidate IV'}`);
}
