...
const { PROTOCOL, IV_CANDIDATES } = require('./d23.constants');

const ALGO = 'aes-128-cbc';

/**
 * Decrypts an AES-CBC hex payload produced by the D23 /api/v1/video endpoint.
 * Key and IV are the plain (UTF-8) protocol string; several known IVs are
 * tried in order to stay compatible across player forks.
 *
 ...
 * validates the response before reading the stream fields.
 *
...
function decryptPayload(payload, options = {}) {
  ...
  if (typeof payload !== 'string') {
    throw new TypeError('D23 payload must be a string');
  }
  const hex = String(payload).replace(/\s+/g, '');
  if (!hex) {
    throw new Error('D23 payload is empty');
  }
  if (hex.length % 2 !== 0) {
    throw new Error('D23 payload is not valid hex (odd length)');
  }
  const ciphertext = Buffer.from(hex, 'hex');
  if (ciphertext.length === 0) {
    throw new Error('D23 payload produced no ciphertext bytes');
  }
  ...
  throw new Error(`D23 payload could not be decrypted: ${lastError ? lastError.message : 'no candidate IV'}`);
}
